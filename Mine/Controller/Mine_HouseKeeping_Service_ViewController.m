...
#define CellID_MineHouseKeepServiceCell @"MineHouseKeepServiceCell"
@interface Mine_HouseKeeping_Service_ViewController ()<UITableViewDelegate, UITableViewDataSource, UINavigationControllerDelegate,UIImagePickerControllerDelegate, TZImagePickerControllerDelegate, MineHouseKeepServiceCellDelegate>
@property (strong, nonatomic) IBOutlet UITableView *tableView;
@property (nonatomic, strong) NSArray *Title_Array;

@property (nonatomic, strong) UIImageView *icon_View;

@property (nonatomic, strong) Mine_MyHouseKeep_Model *My_Model;

@property (strong, nonatomic) IBOutlet UIBarButtonItem *Sure_BT;

@property (nonatomic, strong) NSString *Label_Str;
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:NSStringFromClass([Mine_HouseKeep_Service_Cell class]) bundle:[NSBundle mainBundle]] forCellReuseIdentifier:CellID_MineHouseKeepServiceCell];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        if (indexPath.row == 3) {//服务类型
            Mine_HouseKeep_Service_Cell *cell = [tableView dequeueReusableCellWithIdentifier:CellID_MineHouseKeepServiceCell];
            cell.delegate = self;
            [cell setDataSoureToCell:self.My_Model];
            return cell;
            ...
            }else if (indexPath.row == 2) {//联系地址
                cell.SubTitle_Label.text = self.My_Model.address;
            }else {//我的标签
                if (self.Label_Str.length) {
                    cell.SubTitle_Label.text = self.Label_Str;
                }else {
                    NSString *labelStr = @"";
                    for (NSString *str in self.My_Model.label) {
                        labelStr = [NSString stringWithFormat:@"%@%@", labelStr, str];
                    }
                    cell.SubTitle_Label.text = labelStr;
                }
            }
            return cell;
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    switch (indexPath.row) {
        ...
        case 1://手机号
        {
            ...
            PersonaVC.Number = 2;
            PersonaVC.textStr = self.My_Model.phone;
            MJWeakSelf;
            PersonaVC.MineSetUPPersonaBlack = ^(NSString * _Nonnull TextStr) {
                weakSelf.My_Model.phone = TextStr;
        ...
        case 4://我的标签
        {
            ...
            PersonaVC.Number = 4;
            if (self.Label_Str.length) {
                PersonaVC.textStr = self.Label_Str;
            }else {
                NSString *labelString = @"";
                for (NSString *str in self.My_Model.label) {
                    labelString = [NSString stringWithFormat:@"%@%@", labelString, str];
                }
                PersonaVC.textStr = labelString;
            }
            MJWeakSelf;
            PersonaVC.MineSetUPPersonaBlack = ^(NSString * _Nonnull TextStr) {
                weakSelf.Label_Str = TextStr;
                [weakSelf.tableView reloadData];
            };
            [PersonaVC setHidesBottomBarWhenPushed:YES];
            [self.navigationController pushViewController:PersonaVC animated:YES];
...
#pragma mark----MineHouseKeepServiceCellDelegate
- (void)MineHouseKeepServiceCellButtonClick:(NSString *)index {
    self.My_Model.type = index;
...
- (void)LoadingDataSoure {
    ...
    [[HttpRequest sharedInstance] postWithURLString:URL_centres_serviceInfo parameters:parm success:^(NSDictionary * _Nonnull responseObject) {
        ...
        if ([[responseObject objectForKey:@"status"] intValue]) {
            ...
            if (infoDic) {
                self.My_Model = [Mine_MyHouseKeep_Model mj_objectWithKeyValues:infoDic];
            }
            [self.tableView reloadData];
        }
...
- (void)UploadImageToBacker:(UIImage *)Photo {
    ...
    image.filename = str;
    image.mimeType = @".jpg";
    [MBProgressHUD py_showLoading:@"信息修改中" toView:nil];
...
- (void)UPdataSoureSeleMessage:(NSString *)imageURL {
     ...
     */
    NSMutableDictionary *parm = [[NSMutableDictionary alloc] init];
    [parm setObject:self.My_Model.HouseKeep_ID forKey:@"serviceid"];
    [parm setObject:@"2" forKey:@"type"];
    ...
    [parm setObject:self.My_Model.type forKey:@"status"];
    [parm setObject:self.Label_Str forKey:@"label"];
