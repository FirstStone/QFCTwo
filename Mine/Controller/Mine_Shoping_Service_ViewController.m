...
@interface Mine_Shoping_Service_ViewController ()<UITableViewDelegate, UITableViewDataSource>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0) {
        ...
        if (indexPath.row == 1) {
            cell.SubTitle_Label.text = @"煜星商城";
        }else if (indexPath.row == 2) {
            cell.SubTitle_Label.text = @"18539465047";
        }else if (indexPath.row == 3) {
            cell.SubTitle_Label.text = @"上海市松江区泗泾镇泗泾东区";
        }else if (indexPath.row == 4){
            cell.SubTitle_Label.text = @"服饰箱包";
        }else {
            cell.SubTitle_Label.text = @"8:00-20:00";
        }
        return cell;
    }
}
