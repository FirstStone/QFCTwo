//
//  Mine_Shoping_Service_ViewController.m
//  QFC
//
...
#import "Mine_Shoping_Service_ViewController.h"
#define CellID_MineSetUPCell @"MineSetUPCell"

#define CellID_MineBeginAndEndTimeCell @"MineBeginAndEndTimeCell"
@interface Mine_Shoping_Service_ViewController ()<UITableViewDelegate, UITableViewDataSource, UINavigationControllerDelegate,UIImagePickerControllerDelegate, MineBeginAndEndTimeCellDelegate, PickerViewResultDelegate>
...
@implementation Mine_Shoping_Service_ViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.Title_Array = @[@"店铺头像", @"店铺名称", @"联系方式", @"联系地址", @"经营范围", @"营业时间"];
    ...
    [self.tableView registerNib:[UINib nibWithNibName:NSStringFromClass([Mine_SetUP_Cell class]) bundle:[NSBundle mainBundle]] forCellReuseIdentifier:CellID_MineSetUPCell];
     [self.tableView registerNib:[UINib nibWithNibName:NSStringFromClass([Mine_BeginAnd_EndTimeCell class]) bundle:[NSBundle mainBundle]] forCellReuseIdentifier:CellID_MineBeginAndEndTimeCell];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0) {
        ...
        Mine_BeginAnd_EndTimeCell *cell = [tableView dequeueReusableCellWithIdentifier:CellID_MineBeginAndEndTimeCell];
        [cell setDataSoureToCell:self.Shop_Model];
        cell.delegate = self;
        return cell;
        ...
        if (indexPath.row == 1) {
            cell.SubTitle_Label.text = self.Shop_Model.merchant;
            ...
            cell.SubTitle_Label.text = self.Shop_Model.range;
        }
        return cell;
    }
}
...
//    Mine_ShopingService_Details_ViewController *shopVC = [[Mine_ShopingService_Details_ViewController alloc] init];
...
//    [self.navigationController pushViewController:shopVC animated:YES];
    switch (indexPath.row) {
        ...
        case 1://店铺名
        {
            ...
            PersonaVC.Number = 1;
            PersonaVC.textStr = self.Shop_Model.merchant;
            MJWeakSelf;
            PersonaVC.MineSetUPPersonaBlack = ^(NSString * _Nonnull TextStr) {
                weakSelf.Shop_Model.merchant = TextStr;
        ...
        case 4://经营范围
        {
            Mine_SetUP_PersonalData_Nickname_VC *PersonaVC = [[Mine_SetUP_PersonalData_Nickname_VC alloc] init];
            PersonaVC.Number = 3;
            PersonaVC.textStr = self.Shop_Model.range;
            MJWeakSelf;
            PersonaVC.MineSetUPPersonaBlack = ^(NSString * _Nonnull TextStr) {
                weakSelf.Shop_Model.range = TextStr;
                [weakSelf.tableView reloadData];
            };
            [PersonaVC setHidesBottomBarWhenPushed:YES];
            [self.navigationController pushViewController:PersonaVC animated:YES];
...
#pragma mark----MineBeginAndEndTimeCellDelegate
...
//        self.time_States += 1;
        ...
        self.Shop_Model.beginbusiness = string;
    }else {//结束时间
        NSLog(@"%@", string);
        self.Shop_Model.endbusiness = string;
...
- (void)UPdataSoureSeleMessage:(NSString *)imageURL {
    ...
    [parm setObject:@"3" forKey:@"type"];
    [parm setObject:self.Shop_Model.merchant forKey:@"merchant"];
    [parm setObject:self.Shop_Model.phone forKey:@"phone"];
    [parm setObject:imageURL forKey:@"avatar"];
    [parm setObject:self.Shop_Model.address forKey:@"address"];
    [parm setObject:self.Shop_Model.longitude forKey:@"longitude"];
    [parm setObject:self.Shop_Model.latitude forKey:@"latitude"];
    [parm setObject:self.Shop_Model.range forKey:@"range"];
    [parm setObject:self.Shop_Model.beginbusiness forKey:@"beginbusiness"];
    [parm setObject:self.Shop_Model.endbusiness forKey:@"endbusiness"];
